// backend/utils/sharePointUpload.js
...
async function getSiteId(token) {
  ...
  if (!siteHost || !siteName) throw new Error("Missing SHAREPOINT_SITE or SHAREPOINT_SITE_NAME env vars");
...
async function uploadToSharePoint(file) {
  ...
  // large file: create upload session and chunk
  const sessionResp = await client
    .api(`/sites/${siteId}/drive/root:/${remotePath}:/createUploadSession`)
    .post({ item: { "@microsoft.graph.conflictBehavior": "rename", name: fileName } });
  ...
  while (offset < bufferLength) {
    ...
    const headers = { "Content-Length": contentLength, "Content-Range": contentRange };
