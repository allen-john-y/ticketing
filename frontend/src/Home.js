...
function Home() {
  ...
  useEffect(() => {
    ...
    const prevTickets = JSON.parse(sessionStorage.getItem('prevTickets') || '[]');
    sessionStorage.setItem('prevTickets', JSON.stringify(tickets));

    if (prevTickets.length === 0) return; // First load

    tickets.forEach(current => {
      const previous = prevTickets.find(t => t._id === current._id);
      ...
      if (!previous) {
        if (authority === 'admin') {
          addToast(
            <div>
              <strong>New Ticket #{current.ticketNumber}</strong>
              <br />
              <small>Created by {current.userName || 'Unknown User'}</small>
            </div>,
            'success'
          );
        }
      ...
      if (previous.status !== 'Closed' && current.status === 'Closed') {
        addToast(<strong>Ticket #{current.ticketNumber} Closed</strong>, 'warning');
      }

      // Ticket reopened
      if (previous.status === 'Closed' && current.status !== 'Closed') {
        addToast(<strong>Ticket #{current.ticketNumber} Reopened</strong>, 'info');
      }
    });
  }, [tickets, authority]);
  ...
  return (
    <div style={{ padding: '2rem', maxWidth: '1200px', margin: '0 auto', position: 'relative' }}>
      <style>{`
        ...
        .toast small { opacity: 0.8; font-size: 0.9rem; }
        ...
        {toasts.map(toast => (
          <div key={toast.id} className={`toast ${toast.type}`}>
            <div className="toast-icon">
              {toast.type === 'success' && 'New'}
              {toast.type === 'warning' && 'Closed'}
              {toast.type === 'info' && 'Reopened'}
            toasts}
            </div>
      ...
      <div className="welcome" role="region" aria-label="Welcome">
        <div className="avatar" aria-hidden>
          {profilePhoto ? <img src={profilePhoto} alt={`${userName} profile`} /> : initials}
        </div>
              ...
              {authority === 'admin' && (
                <div style={{ marginTop: 10 }}>
                  <label style={{ fontSize: '0.95rem', color: '#2c3e50', display: 'inline-flex', alignItems: 'center', gap: 8 }}>
                    <input type="checkbox" checked={showMyTickets} onChange={() => setShowMyTickets(prev => !prev)} style={{ transform: 'scale(1.1)' }} />
        ...
        <div className="actions">
          <Link to="/create" style={{ textDecoration: 'none' }}>
            <button className="btn btn-create">Create Ticket</button>
          </Link>
          <Link to="/dashboard" style={{ textDecoration: 'none' }}>
            <button className="btn btn-closed">Closed Tickets</button>
          </Link>
        </div>
      </div>
      ...
      <div className="filters-row" ref={dropdownRef}>
        <div style={{ display: 'flex', gap: 8 }}>
          <button ref={categoryBtnRef} onClick={() => openDropdown('category')} className="filter-btn">
            <span className="filter-dot filter-category" /> Filter by Category
          </button>
          {authority === 'admin' && (
            <button ref={userBtnRef} onClick={() => openDropdown('user')} className="filter-btn">
              <span className="filter-dot filter-user" /> Filter by User
            </button>
          )}
        </div>
      </div>
      ...
      {/* Dropdown */}
      {dropdownOpen && (
        <div className="filter-dropdown" style={{ top: dropdownPos.top, left: dropdownPos.left, minWidth: dropdownPos.width }}>
          ...
          <div className="filter-actions">
            <button onClick={applyFilters} style={{ background: '#27ae60', color: 'white', border: 'none', padding: '8px 12px', borderRadius: 8, cursor: 'pointer', fontWeight: 700 }}>Apply</button>
            <button onClick={() => setDropdownOpen(null)} style={{ background: '#f3f4f6', border: 'none', padding: '8px 12px', borderRadius: 8, cursor: 'pointer', fontWeight: 700 }}>Close</button>
          </div>
        </div>
      )}
        ...
        <div className="applied">
          {[...appliedCategories.map(c => ({ type: 'category', value: c })), ...appliedUsers.map(u => ({ type: 'user', value: u }))].map(({ type, value }) => (
            <div key={value} className="chip">
              <span>{value}</span>
              <button onClick={() => removeFilter(type, value)} style={{ background: 'transparent', color: '#e74c3c', border: 'none', fontWeight: 'bold', cursor: 'pointer' }}>Remove</button>
            </div>
          ))}
          ...
          {openTickets.map(ticket => (
            <Link key={ticket._id} to={`/ticket/${ticket._id}`} style={{ textDecoration: 'none' }}>
              ...
              >
                ...
                {authority === 'admin' && (
                  <>
                    <p style={{ margin: '0.3rem 0', color: '#34495e' }}><strong>Created by:</strong> {ticket.userName || '—'}</p>
                    <p style={{ margin: '0.3rem 0', color: '#34495e' }}><strong>Email:</strong> {ticket.userEmail || '—'}</p>
                  </>
                )}
